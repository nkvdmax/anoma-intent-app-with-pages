  ...
  async function check() {
    try {
      if (chain === "evm-sepolia") {
        ...
        const rec = await client.getTransactionReceipt({ hash: tx });
        setStatus(rec?.status === "success" ? "success" : "pending");
        setDetails(`Block ${rec?.blockNumber}, gasUsed ${rec?.gasUsed}`);
      } else if (chain === "sol-devnet") {
        const conn = new Connection(import.meta.env.VITE_RPC_SOLANA_DEVNET || clusterApiUrl("devnet"));
        const st = await conn.getSignatureStatus(tx);
        const conf = st?.value?.confirmationStatus;
        setStatus(conf ? conf : "pending");
        setDetails(JSON.stringify(st.value || {}, null, 2));
      } else if (chain === "sui-testnet") {
        const provider = new JsonRpcProvider(getFullnodeUrl("testnet"));
        const res = await provider.getTransactionBlock({ digest: tx });
        setStatus(res?.effects?.status?.status?.toLowerCase?.() || "pending");
        setDetails(JSON.stringify(res.effects?.status || {}, null, 2));
      } else {
        setStatus("unknown");
      }
    } catch (e) {
    ...
    <section className="section py-14">
      <div className="card gradient-border p-6">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          <a className="btn" href={links[chain]} target="_blank" rel="noreferrer">Open in Explorer</a>
